...
public sealed class ShotEffects:MonoBehaviour {
 ...
 const int Count=24;LineRenderer[] cores,halos;float[] born,duration;Vector3[] starts,ends,normals;bool[] impact,hostile;int cursor;ParticleSystem flash,sparks,smoke;Material smokeMaterial;
 ...
 void Awake(){
  ...
  flash=Particles("Muzzle flash",24,sparkMaterial,false);sparks=Particles("Impact sparks",96,sparkMaterial,false);
  smokeMaterial=new Material(Shader.Find("Woolly/SoftDust"));smoke=Particles("Barrel smoke",24,smokeMaterial,true);
 }
 ...
 ParticleSystem Particles(string name,int count,Material material,bool grows){
  var g=new GameObject(name);g.transform.SetParent(transform,false);var p=g.AddComponent<ParticleSystem>();p.Stop(true,ParticleSystemStopBehavior.StopEmittingAndClear);var main=p.main;main.loop=true;main.playOnAwake=false;main.simulationSpace=ParticleSystemSimulationSpace.World;main.maxParticles=count;main.startSpeed=0;var emission=p.emission;emission.enabled=false;var shape=p.shape;shape.enabled=false;
 ...
 public void Play(Vector3 origin,Vector3 end,bool impacted=false,Vector3 normal=default,bool enemyShot=false){
  ...
  var direction=(end-origin).normalized;var muzzle=new ParticleSystem.EmitParams{position=origin+direction*.055f,startSize=.25f,startLifetime=.065f,startColor=enemyShot?new Color(1,.4f,.12f):new Color(1,.77f,.18f),rotation=Random.Range(0,360)};flash.Emit(muzzle,1);
  var puff=new ParticleSystem.EmitParams{position=origin,velocity=direction*.35f+Vector3.up*.25f,startSize=.13f,startLifetime=.22f,startColor=new Color(.72f,.66f,.54f,.27f)};smoke.Emit(puff,1);
 }
 ...
 void Impact(int i){
  ...
  var pop=new ParticleSystem.EmitParams{position=origin,startSize=.23f,startLifetime=.10f,startColor=color};sparks.Emit(pop,1);
  for(int k=0;k<5;k++){var e=new ParticleSystem.EmitParams{position=origin,velocity=normals[i]*Random.Range(.7f,1.3f)+Random.insideUnitSphere*1.2f,startSize=Random.Range(.035f,.075f),startLifetime=Random.Range(.13f,.22f),startColor=color,rotation=Random.Range(0,360)};sparks.Emit(e,1);}
 }
 ...
 void OnDestroy(){if(smokeMaterial)Destroy(smokeMaterial);}
}
}
