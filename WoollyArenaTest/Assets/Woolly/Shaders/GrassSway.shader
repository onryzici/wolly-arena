...
V vert(A a){V o;o.positionWS=TransformObjectToWorld(a.positionOS.xyz);float height=max(0,o.positionWS.y-TransformObjectToWorld(float3(0,0,0)).y);float weight=saturate(height/.65);weight*=weight;float phase=dot(TransformObjectToWorld(float3(0,0,0)).xz,float2(1.7,2.3));o.positionWS.x+=sin(_Time.y*1.7+phase)*.035*weight;o.positionWS.z+=cos(_Time.y*1.3+phase)*.022*weight;o.positionCS=TransformWorldToHClip(o.positionWS);o.normalWS=TransformObjectToWorldNormal(a.normalOS);return o;}
