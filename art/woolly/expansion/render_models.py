...
names=['MossMaw','SpineRaptor','HornBrute','StitchReaper','FrostWarden','ThornMatriarch','FrontierRevolver','CopperRepeater','TwinBarrel','GalaxyRelic','ArcCoil','StarLance']
...
for p,power,size in [((1,-4,10),2200,8),((-7,1,6),1400,7)]:
...
bpy.ops.object.camera_add(location=(0,-14,11));cam=bpy.context.object;cam.rotation_euler=(Vector((0,-.5,.5))-cam.location).to_track_quat('-Z','Y').to_euler();cam.data.type='ORTHO';cam.data.ortho_scale=15.5
scene=bpy.context.scene;scene.camera=cam;scene.render.engine='CYCLES';scene.cycles.samples=24;scene.render.resolution_x=1600;scene.render.resolution_y=850;scene.render.resolution_percentage=100;scene.render.filepath=str(root/'art/woolly/expansion/model-review.png');bpy.ops.wm.save_as_mainfile(filepath=str(root/'art/woolly/expansion/ModelReview.blend'));bpy.ops.render.render(write_still=True)
